"""
MPlusForm server trust layer, rc10.6.
...
VERSION = "1.4.2-rc10.6-server-trust-layer"
router = APIRouter()
...
def build_profiles(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    for name, runs in by_player.items():
        ...
        profiles[name] = {
            ...
            "keyRange": [min(as_int(r.get("keyLevel"), 0) for r in last5), max(as_int(r.get("keyLevel"), 0) for r in last5)],
...
async def snapshot_lua():
    ...
    # JSON string inside Lua is safer than hand-emitting complex Lua tables with localized player names.
    raw = json.dumps(snapshot, ensure_ascii=False, separators=(",", ":"))
    return 'MPlusFormServerSnapshotJSON = ' + json.dumps(raw, ensure_ascii=False) + '\n'
